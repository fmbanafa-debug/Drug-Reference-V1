...
// The API Key is intentionally left blank here (key=""). The execution environment securely injects the actual key.
const API_URL = "[https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=](https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=)";
...
const DrugAnalysisModal = ({ drug, onClose }) => {
    ...
    const handleDrugAnalysis = useCallback(async () => {
        ...
        try {
            // FIX: Using empty string for API key to ensure proper runtime injection and avoid 400 error.
            const apiKey = ""; 
            ...
            setError(`Could not retrieve analysis: ${err.message}.`);
        } finally {
            setIsLoading(false);
        }
    }, [drug]);
...
const App = () => {
    ...
    const handleOCRUpload = async (event) => {
        ...
        try {
            ...
            for (let i = 0; i < maxRetries; i++) {
                try {
                    // This fetch call now uses the fixed 'apiKey' variable
                    const response = await fetch(API_URL + apiKey, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(payload)
    ...
    const handleDrugSelect = (drug) => {
        // Toggle selected drug, reset confirmation and analysis states
